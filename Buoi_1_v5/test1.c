...
#include <stdlib.h> // dừng external console thông qua getchar()
...
int main() {
    ...
    printf("\n===== IN THÔNG TIN SÁCH VÀ NGƯỜI DÙNG =====\n");
    printf("INFOR Book 1:\n");
    book_print_info(&books[0]); // In thông tin của cuốn sách đầu tiên

    printf("\nINFOR User 1:\n");
    user_print_info(&users[0]); // In thông tin của người dùng thứ hai
    ...
    // Demonstrate the effect of is_deleted
    printf("\n===== DEMONSTRATING DELETED STATUS =====\n");
    books[0].is_deleted = true; // Mark Book 1 as deleted
    users[0].is_deleted = true; // Mark User 1 as deleted
    
    printf("\n[DEMO] Attempting to borrow Book 1 (deleted) by User 2 (active):\n");
    transaction_borrow(transactions, &transaction_count, &books[0], &users[1]); // Should fail because book is deleted
    
    printf("\n[DEMO] Attempting to borrow Book 9 (active) by User 1 (deleted):\n");
    transaction_borrow(transactions, &transaction_count, &books[8], &users[0]); // Should fail because user is deleted
    
    printf("\n[DEMO] Active transactions after deleting Book 1 and User 1:\n");
    transaction_print_active(transactions, transaction_count); // Book 1 transactions and User 1 transactions won't show if active
    
    printf("\n[DEMO] Transaction history for User 0 (deleted):\n");
    transaction_print_user_history(transactions, transaction_count, &users[0]); // Will show history but with "Deleted User" note

    printf("\nNhan phim bat ky de thoat...");
    getchar(); // Dùng để dừng ở external console khi nó popup, nhớ #include <stdlib.h>
